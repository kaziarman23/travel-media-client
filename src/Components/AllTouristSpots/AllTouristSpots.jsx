...
import { useGetAllSpotsQuery } from "../../Redux/features/api/allSpotsApi";

const AllTouristSpots = () => {
  // RTK Query
  const { data: loadedData = [], isLoading, isError } = useGetAllSpotsQuery();
  ...
  useEffect(() => {
    ...
    if (filterBy !== "all") {
      filtered = filtered.filter(
        (item) => item.category?.toLowerCase() === filterBy.toLowerCase()
      );
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case "popularity":
          return (
            (b.visitors || Math.random() * 1000) -
            (a.visitors || Math.random() * 1000)
          );
        default:
          return 0;
      }
  ...
  return (
    <section className="relative w-full min-h-screen bg-black overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        >
          ...
          <div className="max-w-4xl mx-auto mb-8">
            <p className="text-lg md:text-xl text-gray-300 leading-relaxed">
              ...
              mountains to vibrant cities and ancient wonders, we have curated
              an extensive collection of must-see destinations for every type of
              traveler.
            </p>
          </div>
        ...
        {/* Filters and Search */}
        <div
          className={`mb-12 space-y-6 transition-all duration-1000 delay-300 ${
            isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
          {/* Search Bar */}
        ...
        >
          {filteredData.map((data, index) => (
            ...
            >
              <div
                className={`relative bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-xl hover:border-gray-600 hover:shadow-2xl hover:shadow-blue-500/20 transition-all duration-500 transform-gpu flex flex-col h-full`}
              >
                {/* Image Container */}
                <div
                  className={`relative overflow-hidden ${
                    viewMode === "list"
                      ? "md:w-80 md:h-48 w-full"
                      : "h-64 w-full"
                  } flex-shrink-0`}
                >
                  <img
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  ...
                  {/* Features + Action */}
                  <div className="flex flex-col justify-between items-start mt-auto gap-2">
                    <div className="flex flex-col gap-2">
                      {data.seasonality && (
                        <span className="px-2 py-1 bg-gray-800 text-gray-300 text-xs rounded-lg border border-gray-700 hover:border-blue-500 transition-colors duration-200">
                          <FaClock className="inline mr-1" />
                          {data.seasonality}
                        </span>
                      )}
                      {data.travel_time && (
                        <span className="px-2 py-1 bg-gray-800 text-gray-300 text-xs rounded-lg border border-gray-700 hover:border-green-500 transition-colors duration-200">
                          {data.travel_time}
                        </span>
                      )}
                    </div>
                    ...
                    <Link
                      ...
                      className="flex w-full justify-center items-center space-x-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-all duration-300"
                    >
